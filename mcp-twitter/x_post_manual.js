...
#XRP #HBAR #CryptoAnalysis #Bullish #Utility #MarketStrategy`;
  ...
  // 대표님이 주실 이미지 파일명을 'chart_from_ceo.jpg'로 가정합니다.
  const imagePath = path.join(__dirname, 'chart_from_ceo.jpg');

  if (!fs.existsSync(imagePath)) {
    console.error('ERROR: chart_from_ceo.jpg not found in ' + __dirname);
    return;
  }

  try {
    console.log('Uploading media from representative...');
    ...
    console.log(`SUCCESS: Tweet posted with Representative's chart! ID: ${tweet.data.id}`);
  } catch (error) {
