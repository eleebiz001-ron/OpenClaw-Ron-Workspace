...
const content = `Reports from the recent White House meeting reveal a deepening divide between the banking and crypto sectors. As @EleanorTerrett and @CoinDesk have noted, the atmosphere was thick with tension—traditional banks are "digging in their heels," particularly over the debate on stablecoin yields. The concern of "deposit flight" is real for legacy institutions, while firms like Ripple and Circle argue for market-driven innovation (via @CCN).
...
client.v2.tweet(content)
  .then((val) => console.log('SUCCESS_POST_ID: ' + val.data.id))
  .catch((err) => console.error('CRITICAL_ERROR:', err));
