...
async function fixPostWithTitle() {
  try {
    // 1. Delete the previous one (missing title)
    await client.v2.deleteTweet('2022305080545030583');
    console.log('Deleted post missing title.');
    ...
    // 3. Construct Content with the EXACT title format
    const content = `🤵‍♂️ Strategic Daily Top 10 | Feb 13, 2026 🥇 [News & Trends]

Here are the top 10 financial news stories as of February 13, 2026, based on major market movements, inflation anticipation, policy updates, and sector developments from reliable sources like CNBC, Reuters, Bloomberg, Yahoo Finance, and others:
...
#Economy #Finance #MarketUpdate #SophiaYoon #ArtGallery #XRP #Utility #CantonNetwork`;
    ...
    console.log('Successfully re-posted with EXACT title:', tweet.data.id);
  } catch (error) {
    console.error('Error in re-posting:', error);
  }
}
