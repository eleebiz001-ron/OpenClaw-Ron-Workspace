...
`;
  ...
  try {
    ...
    const postContent = `@realeleanor The "Negative Sprint" narrative is a macro signal we cannot ignore. While the market fixates on the shock, strategic leaders look at the infrastructure that can withstand the turbulence. Clarity is the ultimate hedge against macro uncertainty. 📉🏛️`;

    console.log('--- Generated Content ---\n');
    console.log(postContent);
    console.log('\n--- Posting to X ---');

    const tweet = await twitter.v2.tweet({ text: postContent, reply: { in_reply_to_tweet_id: "2021388219280130234" } });
